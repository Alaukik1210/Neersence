...
- Summarizes results with Ollama for human-friendly answers.
...
import uvicorn
import ollama  # <-- replacing Gemini with Ollama

# ------------------------------
# Ollama Setup
# ------------------------------
# You can change "llama3" to any locally available Ollama model
...
class QueryResponse(BaseModel):
    ...
    gemini_summary: str   # Keeping the same field name to avoid breaking clients
    total_results: int
...
# ------------------------------
def summarize_with_ollama(query_text, results):
    if not results["documents"] or not results["documents"][0]:
...
async def query_argo_profiles(request: QueryRequest):
    ...
    try:
        ...
        gemini_summary = summarize_with_ollama(request.query, results)  # keep same field name
...
def run_cli():
    initialize_resources()
    print("🚀 CLI Mode - Argo Profiles RAG (Ollama)")
    ...
    while True:
        ...
        for rank, (doc, meta, dist) in enumerate(
            ...
            print(f"   Location: ({meta.get('latitude', 'N/A')}, {meta.get('longitude', 'N/A')})")
        ...
        summary = summarize_with_ollama(query_text, results)
        print("\n🤖 Ollama Summary:")
        print(summary)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "cli":
        ...
        print("💡 To run CLI mode: python fastapi_rag_argo_ollama.py cli")
